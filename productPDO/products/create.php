...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(empty($name)) { echo "Product Name is required";
        exit();
    }
    if (empty($category)) {
    ...
    if($stock === false) {
        echo "Enter minimum stock amount";
        exit();
    }
    ...
    if($_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        $sql = "INSERT INTO $tableName(name, description, price, size, color, category, stock, sku, image)
        VALUES(:name, :description, :price, :size, :color, :category, :stock, :sku, :image)";
                ...
                <div class="row g-4">
                    ...
                    <div class="col-12">
                        ...
                        <div class="size-pills">
                            ...
                            foreach ($sizes as $s): ?>
                                <div class="size-pill">
                                    <input type="radio" name="size" id="size_<?php echo $s ?>" value="<?php echo $s ?>">
                        ...
                        <div class="color-swatches">
                            ...
                            foreach ($colors as $name => $hex): ?>
                                <div class="color-swatch" title="<?php echo $name ?>">
                                    <input type="radio" name="color" id="color_<?php echo $name ?>" value="<?php echo $name ?>">
                                    <label for="color_<?php echo $name ?>" style="background:<?php echo $hex ?>"></label>
                                </div>
                            <?php endforeach; ?>

                            <!-- Selected color text display -->
                            <span id="selectedColor" style="font-size:0.8rem;color:var(--muted);margin-left:0.5rem;"></span>
                        </div>
                    </div>
                </div>
            </div>
